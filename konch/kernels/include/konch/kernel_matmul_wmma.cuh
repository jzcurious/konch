...
template <class T>
concept MatmulWMMAKernelConfigKind = KernelConfigKind<T> and requires {
  { T::wmma_m } -> std::convertible_to<index_t>;
  { T::wmma_n } -> std::convertible_to<index_t>;
  { T::wmma_k } -> std::convertible_to<index_t>;
  { T::wmma_colmajor_a } -> std::convertible_to<bool>;
  { T::wmma_colmajor_b } -> std::convertible_to<bool>;
  { T::wmma_colmajor_c } -> std::convertible_to<bool>;
};

struct MatmulWMMAKernelConfigDefault {
  __confield dim3 grid = {1, 1};
  __confield dim3 block = {1, 32};
  __confield std::size_t shmem = 0;
  __confield index_t wmma_m = 16;
  __confield index_t wmma_n = 16;
  __confield index_t wmma_k = 16;
  __confield bool wmma_colmajor_a = false;
  __confield bool wmma_colmajor_b = false;
  __confield bool wmma_colmajor_c = false;
};

__kernel(MatmulWMMAKernelConfigKind) void matmul_wmma(AutoAccessor c,
  ...
  wmma::
      fragment<wmma::accumulator, config::wmma_m, config::wmma_n, config::wmma_k, atom_t>
          fc;
...
KONCH_REGISTER_KERNEL_LAUNCHER(KernelMatmulWMMALauncher, matmul_wmma);
