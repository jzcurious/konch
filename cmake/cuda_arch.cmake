...
function(set_target_minimum_cuda_arch TARGET MINIMUM_CUDA_ARCH)
    ...
    if(GPU_ARCH GREATER "0")
        if(GPU_ARCH GREATER_EQUAL ${MINIMUM_CUDA_ARCH})
            ...
            message(STATUS "CUDA Arch (${TARGET}): Using native compilation (GPU arch: sm_${GPU_ARCH})")
        else()
            set_target_properties(${TARGET} PROPERTIES CUDA_ARCHITECTURES ${MINIMUM_CUDA_ARCH})
            message(STATUS "CUDA Arch (${TARGET}): GPU is too old (sm_${GPU_ARCH}), using sm_${MINIMUM_CUDA_ARCH} as minimum")
        endif()
    else()
        set_target_properties(${TARGET} PROPERTIES CUDA_ARCHITECTURES ${MINIMUM_CUDA_ARCH})
        message(STATUS "CUDA Arch (${TARGET}): No NVIDIA GPU detected, generating PTX for sm_${MINIMUM_CUDA_ARCH}")
    endif()
endfunction()

function(set_minimum_cuda_arch MINIMUM_CUDA_ARCH)
    get_cuda_gpu_arch(GPU_ARCH)
    if(GPU_ARCH GREATER "0")
        if(GPU_ARCH GREATER_EQUAL ${MINIMUM_CUDA_ARCH})
            set(CMAKE_CUDA_ARCHITECTURES "native" PARENT_SCOPE)
            message(STATUS "CUDA Arch: Using native compilation (GPU arch: sm_${GPU_ARCH})")
        else()
            set(CMAKE_CUDA_ARCHITECTURES  ${MINIMUM_CUDA_ARCH} PARENT_SCOPE)
            message(STATUS "CUDA Arch: GPU is too old (sm_${GPU_ARCH}), using sm_${MINIMUM_CUDA_ARCH} as minimum")
        endif()
    else()
        set(CMAKE_CUDA_ARCHITECTURES  ${MINIMUM_CUDA_ARCH} PARENT_SCOPE)
        message(STATUS "CUDA Arch: No NVIDIA GPU detected, generating PTX for sm_${MINIMUM_CUDA_ARCH}")
    endif()
endfunction()
